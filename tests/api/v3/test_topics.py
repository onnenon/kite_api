...
class TopicTest(ForumBaseTest):
    def setUp(self):
        ...
        self.app.post(
            "/api/v3/users", json={"username1": "foo", "password": "testpass"}
        )
        self.app.post(
            "/api/v3/users", json={"username2": "bar", "password": "testpass"}
        )
    ...
    def test_003_get_topics(self):
        resp = self.app.get("/api/v3/topics")
        data = json.loads(resp.data)
        self.logger.debug({"Resp Data": data})
    ...
    def test_004_get_topic_info(self):
        ...
        resp = self.app.get(f"/api/v3/topics/Basketball")
        data = json.loads(resp.data)
        self.logger.debug({"Resp Data": data})
        self.assertEquals(resp.status_code, 200)
    ...
    def test_005_delete_success(self):
        ...
        resp = self.app.delete(f"/api/v3/topics/Basketball")
        self.assertEquals(resp.status_code, 204)

    def test_006_delete_doesnt_exist(self):
        name = "notThere"
        resp = self.app.delete(f"/api/v3/topics/{name}")
        data = json.loads(resp.data)
        self.logger.debug({"Resp Data": data})
    ...
    def test_007_topic_not_found(self):
        ...
        resp = self.app.get(f"/api/v3/topics/{name}")
        data = json.loads(resp.data)
        self.logger.debug({"Resp Data": data})
        self.assertEquals(resp.status_code, 404)
        self.assertEquals(data.get("data").get("title"), f"topic {name} not found")

    @patch("kite.api.v3.topics.Topic.get_all", side_effect=MOCK_REPLY)
    ...
    @patch("kite.api.v3.topics.Topic.get_topic", side_effect=MOCK_REPLY)
    def test_009_mock_get_topic_success(self, mock):
        resp = self.app.get(f"/api/v3/topics/{self}")
        data = json.loads(resp.data)
        self.logger.debug({"Resp Data": data})
        self.assertEquals(resp.status_code, 200)

    @patch("kite.api.v3.topics.Topic.get_topic", return_value=None)
    ...
    def test_011_update_topic_success(self):
        ...
        name = "Basketball"
        resp = self.app.put(
            f"/api/v3/topics/{name}",
            json={"description": "Mostly related to Basketball"},
        )
        data = json.loads(resp.data)
        self.logger.debug({"Resp Data": data})
        self.assertEquals(resp.status_code, 200)
